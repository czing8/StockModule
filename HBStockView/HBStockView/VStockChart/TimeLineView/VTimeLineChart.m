...
- (NSArray *)drawViewWithXPosition:(CGFloat)xPosition timeLineGroup:(VTimeLineGroup *)timeLineGroup maxValue:(CGFloat)maxValue minValue:(CGFloat)minValue {
    ...
    // 转换为实际坐标
    [self convertToPositionsWithXPosition:xPosition drawLineModels:timeLineGroup.lineModels maxValue:maxValue minValue:minValue];
...
- (NSArray *)convertToPositionsWithXPosition:(CGFloat)startX drawLineModels:(NSArray <VTimeLineModel *>*)drawLineModels  maxValue:(CGFloat)maxValue minValue:(CGFloat)minValue {
    ...
    [drawLineModels enumerateObjectsUsingBlock:^(VTimeLineModel * model, NSUInteger idx, BOOL * _Nonnull stop) {
