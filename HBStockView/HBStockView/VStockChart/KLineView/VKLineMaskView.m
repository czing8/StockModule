...
- (void)drawDashLine {
    ...
    CGContextAddLineToPoint(ctx, x, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - VStockLineDayHeight/2.f);
    ...
    CGContextAddArc(ctx, x, self.stockScrollView.frame.origin.y + self.selectedPosition.closePoint.y, VStockPointRadius, 0, 2 * M_PI, 0);
    ...
    if (x + textRect.size.width/2.f + 2 > CGRectGetMaxX(self.stockScrollView.frame)) {
        ...
        CGContextFillRect(ctx, CGRectMake(CGRectGetMaxX(self.stockScrollView.frame) - 4 - textRect.size.width, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - VStockLineDayHeight, textRect.size.width + 4, textRect.size.height + 4));
        ...
        CGContextFillRect(ctx, CGRectMake(x-textRect.size.width/2.f, self.stockScrollView.frame.origin.y + self.stockScrollView.bounds.size.height - VStockLineDayHeight, textRect.size.width + 4, textRect.size.height + 4));
    ...
    CGContextSetFillColorWithColor(ctx, [UIColor selectedRectBgColor].CGColor);
    CGContextFillRect(ctx, CGRectMake(VStockScrollViewLeftGap - priceRect.size.width - 4, self.stockScrollView.frame.origin.y + self.selectedPosition.closePoint.y - priceRect.size.height/2.f - 2, priceRect.size.width + 4, priceRect.size.height + 4));
    [priceText drawInRect:CGRectMake(VStockScrollViewLeftGap - priceRect.size.width - 4 + 2, self.stockScrollView.frame.origin.y + self.selectedPosition.closePoint.y - priceRect.size.height/2.f, priceRect.size.width, priceRect.size.height) withAttributes:attribute];
}
